...
let content = fs.readFileSync('./src/pages/announcements/announcementList.yaml', {encoding: "utf-8"})
...
module.exports = [
	...
	{url: '/rules/', priority: '0.80', mainContent: ['./src/pages/markdown/RulesEN.md']},
	...
	{url: '/announcements/', priority: '0.70', mainContent: glob.sync('./src/pages/announcements/**.*')},
	...announcementList.posts.map(post => ({
		url: `/announcements/${post.slug || removeMdSuffix(post.file)}`,
		priority: '0.60',
		mainContent: [`./src/pages/${removeMdSuffix(post.file)}.md`]
	})),
